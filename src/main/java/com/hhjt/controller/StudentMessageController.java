...
public class StudentMessageController {
    ...
    private static final Logger log = LoggerFactory.getLogger(StudentMessageController.class);
    ...
    public Map<String, Object> sendMessage(@RequestParam Long receiverId,
        ...
        try {
            ...
            message.setMessageType("teacher".equals(receiverType) ? "STUDENT" : "STUDENT");
            ...
            log.info("学生{}发送消息给{} {}", userId, receiverType, receiverId);
        } catch (Exception e) {
            result.put("success", false);
            result.put("message", e.getMessage());
            log.error("发送消息失败", e);
        ...
        try {
            ...
            List<Student> classmates = studentMapper.selectList(
                new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<Student>()
                    ...
                    .ne(Student::getUserId, userId)
                    .orderByAsc(Student::getStudentNo)
            );

            List<Map<String, Object>> classmateList = classmates.stream()
                .map(s -> {
                    Map<String, Object> map = new HashMap<>();
            ...
            data.put("classmates", classmateList);
